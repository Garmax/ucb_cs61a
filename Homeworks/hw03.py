...
def flatten(lst):
    ...
    res=[]
    for i in lst:
      res=res+flatten(i) if type(i)==list else res+[i]
    return res
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
      ...
      newBranch=[replace_leaf(i,old,new) for i in branches(t)]
...
def totals_tree(m):
    ...
    bran=[]
    if is_weight(m):
      bran+=tree(total_weight(m))
    else:
      bran+=tree(total_weight(m),[totals_tree(end(left(m))),totals_tree(end(right(m)))])
    return bran
...
""" 
The logician Alonzo Church invented a system of representing non-negative
integers entirely using functions. The purpose was to show that functions
are sufficient to describe all of number theory: if we have functions, we
do not need to assume that numbers exist, but instead we can invent them.

Your goal in this problem is to rediscover this representation known as 
Church numerals. Here are the definitions of zero, as well as a function 
that returns one more than its argument: 
"""
...
""" 
Church numerals are a way to represent non-negative integers via repeated
function application. The definitions of zero, one, and two show that each
numeral is a function that takes a function and repeats it a number of times
on some argument x.

The church_to_int function reveals how a Church numeral can be mapped to
our normal notion of non-negative integers using the increment function.

Addition of Church numerals is function composition of the functions of x,
