...
def accumulate(combiner, base, n, term):
    ...
    sum,i=term(n),n
    while i>1:
        sum,i=combiner(sum,term(i-1)),i-1
    return combiner(sum,base)
...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(k,dir,storage):
        if k==n:
            return storage
        else:
            if k%7==0 or num_sevens(k)>0:
                return helper(k+1,-dir,storage-dir)
            else:
                return helper(k+1,dir,storage+dir)
    return helper(1,1,1)
...
def count_change(amount):
    ...
    """
    "*** YOUR CODE HERE ***" 
    def helper2(remain,largest_change):
        if remain<0 or largest_change<1: return 0
        elif remain==0 or largest_change==1: return 1
        else:
            return helper2(remain-largest_change,largest_change)\
                +helper2(remain,largest_change//2)
    
    return helper2(amount,upperbound(amount))
