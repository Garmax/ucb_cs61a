...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    #Might be better ways to return strings
    at=[]
    def wd(amount,pw):
        nonlocal balance,at
        if len(at)==3:
            return "Your account is locked. Attempts: ['"+at[0]+"'"\
                +', '+"'"+at[1]+"'"+', '+"'"+at[2]+"']"
        if pw!=password:
            at.append(pw)
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.current_balance<self.price:
            k=self.price-self.current_balance
            return 'You must deposit ${0} more.'.format(k)
        if self.current_balance>self.price:
            k=self.current_balance-self.price
            self.current_balance=0
            self.stock-=1
            return 'Here is your {0} and ${1} change.'.format(self.food,k)
        self.current_balance=0
        self.stock-=1
    ...
    def restock(self,ammount):
        self.stock+=ammount
        return 'Current {0} stock: {1}'.format(self.food,self.stock)
...
def remove_all(link , value):
    ...
    "*** YOUR CODE HERE ***"
    i=link
    while i.rest is not Link.empty:
        if i.rest.first==value:
            i.rest=i.rest.rest
        else:
            i=i.rest
